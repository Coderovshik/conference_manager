...
# Test 2: Try to create conference without auth
echo -e "\n${GREEN}Test 2: Create conference without auth${NC}"
...
# Test 3: Create conference with auth
echo -e "\n${GREEN}Test 3: Create conference with auth${NC}"
...
GET_RESPONSE=$(curl -s -X GET "${CONF_URL}/${CONFERENCE_ID}" \
  -H "Authorization: Basic $(echo -n 'testuser:testpass' | base64)")
echo "Response: $GET_RESPONSE"

# Test 5: Update conference
echo -e "\n${GREEN}Test 5: Update conference${NC}"
UPDATE_RESPONSE=$(curl -s -X PUT "${CONF_URL}/${CONFERENCE_ID}" \
  -H "Content-Type: application/json" \
  -H "Authorization: Basic $(echo -n 'testuser:testpass' | base64)" \
  -d '{
...
# Test 6: Get all conferences
echo -e "\n${GREEN}Test 6: List all conferences${NC}"
LIST_RESPONSE=$(curl -s -X GET "${CONF_URL}/" \
  -H "Authorization: Basic $(echo -n 'testuser:testpass' | base64)")
echo "Response: $LIST_RESPONSE"

# Test 7: Try to get non-existent conference
echo -e "\n${GREEN}Test 7: Get non-existent conference${NC}"
NOT_FOUND_RESPONSE=$(curl -s -X GET "${CONF_URL}/non-existent-id" \
  -H "Authorization: Basic $(echo -n 'testuser:testpass' | base64)")
echo "Response: $NOT_FOUND_RESPONSE"

# Test 8: Delete conference
echo -e "\n${GREEN}Test 8: Delete conference${NC}"
DELETE_RESPONSE=$(curl -s -X DELETE "${CONF_URL}/${CONFERENCE_ID}" \
  -H "Authorization: Basic $(echo -n 'testuser:testpass' | base64)")
echo "Response: $DELETE_RESPONSE"

# Test 9: Verify deletion
echo -e "\n${GREEN}Test 9: Verify deletion${NC}"
GET_DELETED_RESPONSE=$(curl -s -X GET "${CONF_URL}/${CONFERENCE_ID}" \
  -H "Authorization: Basic $(echo -n 'testuser:testpass' | base64)")
